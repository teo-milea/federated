...
class ExecutorTest(absltest.TestCase):
  ...
  def test_create_compute_int32(self):
    ...
    int_val = asyncio.get_running_loop().run_until_complete(
    ...
    result = asyncio.get_running_loop().run_until_complete(int_val.compute())
    self.assertEqual(result, 10)

  def test_create_compute_2xint32_struct(self):
    ex = executor.XlaExecutor()
    x_val = asyncio.get_running_loop().run_until_complete(
        ex.create_value(10, np.int32))
    y_val = asyncio.get_running_loop().run_until_complete(
        ex.create_value(20, np.int32))
    struct_val = asyncio.get_running_loop().run_until_complete(
    ...
    result = asyncio.get_running_loop().run_until_complete(struct_val.compute())
  ...
  def test_create_and_invoke_noarg_comp_returning_int32(self):
    ...
    comp_pb = xla_serialization.create_xla_tff_computation(
        xla_comp, [], comp_type)
    ex = executor.XlaExecutor()
    comp_val = asyncio.get_running_loop().run_until_complete(
    ...
    self.assertEqual(result, 10)
    call_val = asyncio.get_running_loop().run_until_complete(
    ...
    result = asyncio.get_running_loop().run_until_complete(call_val.compute())
    self.assertEqual(result, 10)
  ...
  def test_add_numbers(self):
    ...
    result = asyncio.get_running_loop().run_until_complete(_compute_fn())
    self.assertEqual(result, 50)
  ...
  def test_selection(self):
    ...
    by_index_val = asyncio.get_running_loop().run_until_complete(
        ex.create_selection(struct_val, index=0))
    self.assertEqual(by_index_val.internal_representation, 10)
    by_name_val = asyncio.get_running_loop().run_until_complete(
