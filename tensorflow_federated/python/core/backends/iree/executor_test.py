...
class ExecutorTest(tf.test.TestCase):
  ...
  def test_float(self):
    ...
    float_val = asyncio.get_running_loop().run_until_complete(
    ...
    result = asyncio.get_running_loop().run_until_complete(float_val.compute())
  ...
  def test_constant_computation(self):
    ...
    with self.assertRaises(TypeError):
      asyncio.get_running_loop().run_until_complete(comp_val.compute())

    result_val = asyncio.get_running_loop().run_until_complete(
    ...
    result = asyncio.get_running_loop().run_until_complete(result_val.compute())
    self.assertEqual(result, 1000.0)
  ...
  def test_add_one(self):
    ...
    arg_val = asyncio.get_running_loop().run_until_complete(
        ex.create_value(10.0, tf.float32))
    result_val = asyncio.get_running_loop().run_until_complete(
